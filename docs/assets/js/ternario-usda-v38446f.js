...
function bindDomNow(){
 const direct=(id,fn)=>{const el=$(id);if(!el)return;el.onclick=(ev)=>{ev.preventDefault();ev.stopPropagation();fn()}};
 direct('itaTernarioAjuda',help);
 direct('itaTernarioCiencia',science);
 direct('itaTernarioAjudaMain',help);
 direct('itaTernarioBibliografiaMain',bibliography);
 direct('itaTernarioScienceFromHelp',science);
 direct('itaTernarioBibliografiaFromHelp',bibliography);
 direct('itaTernarioScienceFromRefs',science);
 document.querySelectorAll('[data-ternario-return]').forEach(el=>{el.onclick=(ev)=>{ev.preventDefault();ev.stopPropagation();returnToDiagram()}});
 document.querySelectorAll('[data-ternario-mode]').forEach(el=>{el.onclick=(ev)=>{ev.preventDefault();ev.stopPropagation();setMode(el.dataset.ternarioMode)}});
 const n=$('ternarioNormalize');if(n)n.onclick=(ev)=>{ev.preventDefault();ev.stopPropagation();normalize()};
 const c=$('ternarioClear');if(c)c.onclick=(ev)=>{ev.preventDefault();ev.stopPropagation();clearValues()};
 const back=document.querySelector('#ternarioModal [data-close="ternarioModal"]');if(back)back.onclick=(ev)=>{ev.preventDefault();ev.stopPropagation();if(returnToDiagram())return;closeToHub()};
 ['ternarioSand','ternarioSilt','ternarioClay','ternarioSample'].forEach(id=>{const el=$(id);if(el&&!el.dataset.ternarioBound){el.dataset.ternarioBound='1';el.addEventListener('input',update);el.addEventListener('change',update)}});
 const canvas=$('ternarioCanvas');if(canvas&&!canvas.dataset.ternarioBound){canvas.dataset.ternarioBound='1';canvas.addEventListener('pointerdown',e=>{drag=true;canvas.setPointerCapture?.(e.pointerId);pointer(e)});canvas.addEventListener('pointermove',e=>{if(drag)pointer(e)});canvas.addEventListener('pointerup',()=>drag=false);canvas.addEventListener('pointercancel',()=>drag=false);canvas.addEventListener('pointerleave',()=>drag=false)}
}

function open(){init();bindDomNow();const hub=$('ferramentasModal');if(hub){hub.classList.remove('open');hub.setAttribute('aria-hidden','true')}const m=$('ternarioModal');if(!m)return;m.classList.add('open');m.setAttribute('aria-hidden','false');returnToDiagram(true);requestAnimationFrame(()=>{background=null;update()})}
...
function help(){bindDomNow();const m=$('ternarioModal');if(!m?.classList.contains('open'))open();showSubview('help')}
function science(){bindDomNow();const m=$('ternarioModal');if(!m?.classList.contains('open'))open();showSubview('science')}
function bibliography(){bindDomNow();const m=$('ternarioModal');if(!m?.classList.contains('open'))open();showSubview('bibliography')}
...
function init(){if(initialized){bindDomNow();return}initialized=true;document.addEventListener('click',onClick,true);document.addEventListener('input',onInput,true);document.addEventListener('change',onInput,true);document.addEventListener('pointerdown',onPointerDown,true);document.addEventListener('pointermove',onPointerMove,true);document.addEventListener('pointerup',onPointerEnd,true);document.addEventListener('pointercancel',onPointerEnd,true);window.addEventListener('resize',()=>{background=null;draw()});if($('ternarioModal')){bindDomNow();update()}}

init();
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>update(),{once:true});
window.ITA_TERNARIO={version:VERSION,open,help,science,bibliography,returnToDiagram,closeToHub,classify,update,normalize,clear:clearValues,setMode,init:()=>{init();bindDomNow();update()}};document.documentElement.dataset.itaTernario=VERSION;
})();
